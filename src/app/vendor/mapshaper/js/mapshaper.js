  ...
  */
  ...
  internal.getCRS = function (str) {
    ...
    if (typeof defn == 'function') {
      ...
      try {
        /** custom - Here require('') not worked,
         * so replace with mproj as module from index.ts, 
         * same change in all places in this js */
        p = mproj.pj_init(defn);
      } catch (e) {
        stop('Unable to use projection', defn, '(' + e.message + ')');
      }
    }
    return mproj.pj_init(defn);
  };
        ...
        internal.initProjLibrary(opts, function () {
          ...
          try {
            targets.forEach(function (targ) {
              ...
              /** custom - Here getting input through EPSG:4326*projStr and then split the str */
              var _projData = destArg.split('*');
              destArg = _projData[0];
              targ.dataset.info.prj = _projData[1];
              if (opts.from) {
