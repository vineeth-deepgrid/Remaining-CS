(function () {
  ...
  */
  ...
  function mproj_read_lib_anycase(libFile) {
    ...
    var pathUC = './nad/epsg',
      pathLC = './nad/EPSG';
      /** custom - removed require('fs') and getting epsg content from assets or default 4326 */
    /* fetch('/assets/epsg/epsg')
  ...
  function pj_transform_point(srcdefn, dstdefn, p) {
    ...
    /** custom - in all dstdefn. replaced with dstdefn.crs, this changes are in all mproj.js*/
    if (dstdefn.crs.is_latlong) {
      xx[0] *= RAD_TO_DEG;
      yy[0] *= RAD_TO_DEG;
    }
    p[0] = xx[0];
  ...
  function pj_transform(srcdefn, dstdefn, xx, yy, zz) {
    ...
    if (dstdefn.crs.has_geoid_vgrids && zz) {
      fatal("vgrid transformation not supported");
    }

    if (dstdefn.crs.from_greenwich !== 0) {
      for (i = 0; i < point_count; i++) {
        if (xx[i] != HUGE_VAL) {
          xx[i] -= dstdefn.crs.from_greenwich;
        }
      }
    }

    if (dstdefn.crs.is_geocent) {
      if (!zz) {
        error(PJD_ERR_GEOCENTRIC);
      }
      pj_geodetic_to_geocentric(dstdefn.crs.a_orig, dstdefn.crs.es_orig, xx, yy, zz);

      if (dstdefn.crs.fr_meter != 1) {
        for (i = 0; i < point_count; i++) {
          if (xx[i] != HUGE_VAL) {
            xx[i] *= dstdefn.crs.fr_meter;
            yy[i] *= dstdefn.crs.fr_meter;
          }
        }
      }
    } else if (!dstdefn.crs.is_latlong) {
      if (dstdefn.crs.fwd3d) {
    ...
    } else if (dstdefn.crs.is_latlong && dstdefn.crs.is_long_wrap_set) {
      for (i = 0; i < point_count; i++) {
        if (xx[i] == HUGE_VAL) continue;
        while (xx[i] < dstdefn.crs.long_wrap_center - M_PI) {
          xx[i] += M_TWOPI;
        }
        while (xx[i] > dstdefn.crs.long_wrap_center + M_PI) {
          xx[i] -= M_TWOPI;
    ...
    if (dstdefn.crs.vto_meter != 1 && zz) {
      for (i = 0; i < point_count; i++) {
        zz[i] *= dstdefn.crs.vfr_meter;
      }
    }
    if (dstdefn.crs.axis != 'enu') {
      pj_adjust_axis(dstdefn.crs.axis, true, xx, yy, zz);
    }
  ...
  function pj_datum_transform(srcdefn, dstdefn, xx, yy, zz) {
    ...
    if (srcdefn.datum_type == PJD_UNKNOWN || dstdefn.crs.datum_type == PJD_UNKNOWN) {
      return;
    }
    ...
    dst_a = dstdefn.crs.a_orig;
    dst_es = dstdefn.crs.es_orig;
    ...
    if (dstdefn.crs.datum_type == PJD_GRIDSHIFT) {
    ...
    if (src_es != dst_es || src_a != dst_a ||
      ...
      dstdefn.crs.datum_type == PJD_3PARAM || dstdefn.crs.datum_type == PJD_7PARAM) {
      ...
      if (dstdefn.crs.datum_type == PJD_3PARAM || dstdefn.crs.datum_type == PJD_7PARAM) {
  ...
  function pj_compare_datums(srcdefn, dstdefn) {
    if (srcdefn.datum_type != dstdefn.crs.datum_type) return false;
    if (srcdefn.a_orig != dstdefn.crs.a_orig ||
      Math.abs(srcdefn.es_orig - dstdefn.crs.es_orig) > 0.000000000050) {
      /* the tolerance for es is to ensure that GRS80 and WGS84 are considered identical */
      return false;
    }
    if (srcdefn.datum_type == PJD_3PARAM) {
      return (srcdefn.datum_params[0] == dstdefn.crs.datum_params[0] &&
        srcdefn.datum_params[1] == dstdefn.crs.datum_params[1] &&
        srcdefn.datum_params[2] == dstdefn.crs.datum_params[2]);
    }
    if (srcdefn.datum_type == PJD_7PARAM) {
      return (srcdefn.datum_params[0] == dstdefn.crs.datum_params[0] &&
        srcdefn.datum_params[1] == dstdefn.crs.datum_params[1] &&
        srcdefn.datum_params[2] == dstdefn.crs.datum_params[2] &&
        srcdefn.datum_params[3] == dstdefn.crs.datum_params[3] &&
        srcdefn.datum_params[4] == dstdefn.crs.datum_params[4] &&
        srcdefn.datum_params[5] == dstdefn.crs.datum_params[5] &&
        srcdefn.datum_params[6] == dstdefn.crs.datum_params[6]);
    }
    if (srcdefn.datum_type == PJD_GRIDSHIFT) {
      return pj_param(srcdefn.params, "snadgrids") ==
        pj_param(dstdefn.crs.params, "snadgrids");
    }
    return true;
  }
