    ...
    animations: [
        trigger('rotateWing', [
            ...
            transition(':leave', [
                animate('300ms cubic-bezier(0.680, -0.550, 0.265, 1.550)', style({ transform: 'rotate({{startAngles}}deg) scale(1)' })),
                animate('300ms 300ms cubic-bezier(0.680, -0.550, 0.265, 1.550)', style(
                    { transform: 'rotate({{startAngles}}deg) scale(0)' }))
            ]),
        ]),
        trigger('scaleWing', [
...
export class MenuWingComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
    ...
    @Output() public wingHoveredOut = new EventEmitter<IMenuWing>();
    ...
    public ngOnInit() {
        console.log('IN MENU WING ONINIT');
        console.log(this);
    ...
    public onMouseOut(): void {
        ...
        this.wingHoveredOut.emit(this.wing);
    }
    ...
    private setWingIconTransformStyle(deg: number): void {
        if (this.menuConfig.showIcons || this.menuConfig.onlyIcons) {
            this.renderer.setStyle(this.wingIconElm.nativeElement, 'transform', 'translate('
                + this.iconX + 'px, ' + this.iconY + 'px) rotate(' + (this.rotateDeg + deg) * -1 + 'deg)');
            this.renderer.setStyle(this.wingIconElm.nativeElement, '-webkit-transform', 'translate('
                + this.iconX + 'px, ' + this.iconY + 'px) rotate(' + (this.rotateDeg + deg) * -1 + 'deg)');
            this.renderer.setStyle(this.wingIconElm.nativeElement, '-ms-transform', 'translate('
                + this.iconX + 'px, ' + this.iconY + 'px) rotate(' + (this.rotateDeg + deg) * -1 + 'deg)');
            this.renderer.setStyle(this.wingIconElm.nativeElement, '-moz-transform', 'translate('
                + this.iconX + 'px, ' + this.iconY + 'px) rotate(' + (this.rotateDeg + deg) * -1 + 'deg)');
            this.renderer.setStyle(this.wingIconElm.nativeElement, '-o-transform', 'translate('
                + this.iconX + 'px, ' + this.iconY + 'px) rotate(' + (this.rotateDeg + deg) * -1 + 'deg)');
        }
        return;
    }
