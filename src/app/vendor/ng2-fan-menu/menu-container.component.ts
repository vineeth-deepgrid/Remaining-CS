...
export class MenuContainerComponent implements OnInit, OnDestroy {
    ...
    // Emit the wing that is been mouse over
    @Output() public onWingHoveredOut = new EventEmitter<IMenuWing>();
    ...
    public top: number=0; // for draggingMenu animation
    public left: number=0; // for draggingMenu animation
    ...
    public ngOnInit() {
        console.log('IN MENU CONTAINER ONINIT');
        console.log(this);
        ...
        this.menuSpunSubscriptionId =
            this.spinService.wingSpun.subscribe((data: number) => {
                console.log('IN WIN SPIN');
                console.log(data);
                this.spinMenu(data);
            });
    }
    ...
    /**
     * Emit the wing that has been mouse over out
     * */
    ...
    public toggleSpinningState(state: boolean): void {
        console.log('toggleSpinningState');
        console.log(state);
        if (state) {
    ...
    private calculateSvgPath(): void {
        ...
        const x1 = Math.floor(radius * Math.cos(Math.PI * (360 - angle / 2) / 180));
        const y1 = Math.floor(radius / 2 + radius * Math.sin(Math.PI * (360 - angle / 2) / 180));
        const x2 = Math.floor(radius * Math.cos(Math.PI * (angle / 2) / 180));
        const y2 = Math.floor(radius / 2 + radius * Math.sin(Math.PI * (angle / 2) / 180));
        const a1 = Math.floor(innerRadius * Math.cos(Math.PI * (360 - angle / 2) / 180));
        const b1 = Math.floor(radius / 2 + innerRadius * Math.sin(Math.PI * (360 - angle / 2) / 180));
        const a2 = Math.floor(innerRadius * Math.cos(Math.PI * (angle / 2) / 180));
        const b2 = Math.floor(radius / 2 + 1 + innerRadius * Math.sin(Math.PI * (angle / 2) / 180));
