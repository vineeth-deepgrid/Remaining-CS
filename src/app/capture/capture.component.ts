...
export class CaptureComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit {
  ...
  fucntionClick(e){
    ...
    console.log(this.showNotesAndFilePicker,"check capture-notes enabled")
    if(this.showNotesAndFilePicker){
      console.log("i got hitted true")
      localStorage.setItem('refresh',`${`EPSG:${e}`}`)
    }
  }
  ngOnInit(): any {
  ...
  showOrCloseLocationOnMap(note: any, operation = '', zoomToSite = true): any {
    console.log(note,"check the note location")
    this.savedNotes.map((x)=>{
      if(note.observationInstanceId === x.observationInstanceId){
        console.log(x,"check matched note")
      }
    })
    var projection;
    const projCode = localStorage.getItem('projCode')
    console.log(projCode,"check the exact projection")
    this.basemapService.projectionsList.map((x)=>{
      if(x.name === projCode){
        projection = x.projection
      }
    })
    console.log(projection,"check the exact projection")
    if(this.globalNotes.length === 0){
      this.globalNotes = note.latitudeLongitude
    }
    console.log(this.globalNotes,"check the gloabl object of notes")
    const checkObj = []
    ...
    if (note.latitudeLongitude.length === 2) {
      ...
      let i = 0;
      const coOrds = [];
      console.log(this.globalNotes,"check global notes")
      while ( i < this.globalNotes.length ) {
        try{
          const tempArray = [Number(this.globalNotes[i]), Number(this.globalNotes[i + 1])];
      ...
      console.log(coOrds,"check the input coordinates before cs1 transformation")
      console.log(projection,"check the projection of cs1")
      console.log(this.basemapService.projectionsList[8].projection,"check the input proj")
      // const current_projection =   this.projectionCode ? this.projectionCode :  this.basemapService.getCurrentBasemap().getView().getProjection()
      // console.log(current_projection,"check the current proj")
      console.log(note,"check the input coordinates default")
      if(projection){
        if(projection !== this.basemapService.projectionsList[8].projection){
          console.log(" i am not matched")
          console.log(coOrds,"check the coordinates when not matched")
          for(let i=0;i<coOrds.length;i++){
           ...
           console.log(checkObj,"checkkkkkkkkkkkk.........")
      ...
      // currentContextInfo.site.latitudeLongitude = output_coord

    }
    console.log(currentContextInfo.site.latitudeLongitude,"check the context")
   
    console.log(currentContextInfo,"check")
    console.log(geometryData,"check the geometry data")
...
console.log(data,"check the data object")
    ...
    this.basemap.getLayers().forEach(layerObj => {
      if (layerObj !== undefined) {
        if (layerObj.values_.name === data.name) {
          console.log(layerObj,"check  the layers ")
          // this.basemap.removeLayer(layerObj);
          layerFound = true;
          addedLayerObj = layerObj;
        }else{
          console.log("i got maniplateds")
        }
      }else{
        console.log("slow it may possible")
      }
    });

    console.log(this.basemap.getLayers(),"check the layers.....")
    let visible = false;
    // if (layerFound) {
    ...
    // if(projection){
    //   console.log(currentContextInfo,"check the context before passing")
    //       if (note.latitudeLongitude.length === 2) {
    //     console.log('ADDING POINT');
    //     this.notePadService.reDrawPointOrPolygonOnMap(this.notePadService.shapeDrawType.POINT,
    //                                                 data, operation !== 'awareness' && zoomToSite, null, currentContextInfo, siteParams);
  ...
  viewSiteDetails(note): void{
    this.notePadService.getSelectedNoteId(note.observationInstanceId)
    this.countNotes = this.countNotes + 1
    console.log(this.countNotes,this.savedNotes,"why count is not working...")
console.log(note,"check note........")

    // this.saveNoteGlobal = note
    console.log("note",this.saveNoteGlobal,this.basemapService.getCurrentBasemap().getView().getProjection())
    console.log("global",this.saveNoteGlobal,this.basemapService.getCurrentBasemap().getView().getProjection())
    console.log('IN viewSiteDetails',note);
    ...
    console.log(this);
    console.log(this.selectedNote);
    console.log("check me if i got hit 8")
    // this.showOrCloseLocationOnMap(note,'close');
