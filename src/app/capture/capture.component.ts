...
export class CaptureComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit {
  ...
  showOrCloseLocationOnMap(note: FuseEarthSite, operation = '', watchOnPolygonChanges = null): any {
    ...
    console.log(note,this.storeCoordinates,"note and storecoordinates check")
    ...
    if (note.latitudeLongitude.length === 2) {  
      ...
      this.geometryData.coordinates = isPolygon ? [coOrds] : coOrds;

    }
  
if(this.epsgDetection){
    this.projCode = this.epsgDetection
    console.log(this.projCode,"ciusuhcowhchcoa")
    output_coord = []
   this.basemapService.projectionsList.map((x)=>{
      console.log(x,this.projCode, "check the x in capture")
      if(x.name === this.projCode){
        this.projection = x.projection
      }
    })
console.log(this.projection,"check projection outside")
    if (note.latitudeLongitude.length === 2) {   
      pointArray = [Number(note.latitudeLongitude[0]),Number(note.latitudeLongitude[1])]
      ...
      this.geometryData.coordinates =  transformed_coord
      ...
      this.geometryData.coordinates = isPolygon ? [output_coord] : output_coord;
      // currentContextInfo.site.latitudeLongitude = checkObj
      // currentContextInfo.site.latitudeLongitudeToShow = checkObj
    }
    console.log(this.geometryData,currentContextInfo,"ytcxdxfgvb")
   ...
   if (operation === '' || operation === 'zoom') {
    ...
    if (note.latitudeLongitude.length === 2) {
      ...
      this.notePadService.reDrawPointOrPolygonOnMapForCapture(this.notePadService.shapeDrawType.POINT, data,
              operation === 'zoom', watchOnPolygonChanges, currentContextInfo);
              visible = true;
      ...
      this.notePadService.reDrawPointOrPolygonOnMapForCapture(
              ...
              data, operation === 'zoom', watchOnPolygonChanges, currentContextInfo);
       visible = true
    }
    // this.storeObjectCount = true
  }
  const siteIndex = this.storeCoordinates.findIndex(site => site.observationInstanceId === note.observationInstanceId);
  if (siteIndex !== -1) {
    this.storeCoordinates[siteIndex].visible = visible;
  }
}
    this.changeEpsgCode.subscribe(epsg => {
      this.projCode = epsg
      console.log(this.projCode,"ciusuhcowhchcoa")
      output_coord = []
     this.basemapService.projectionsList.map((x)=>{
